...
function view_update(view, cam)
{
	...
	// Trackpad orbit
	var trackpad_orbit = (mouse_trackpad_x != 0 || mouse_trackpad_y != 0)
	if (((window_busy = "" || window_busy = "place") && content_mouseon) && trackpad_orbit)
	{
		if (!cam || (cam.value[e_value.CAM_ROTATE] && editcamobj))
		{
			if (cam)
			{
				action_tl_select_single(cam)
			...
			camera_control_rotate_delta(cam, -(mouse_trackpad_x / 4), mouse_trackpad_y / 4)
		}
	}
	...
	var zoom_delta = 0
	if (!trackpad_orbit)
		zoom_delta = (mouse_wheel_smooth != 0 ? mouse_wheel_smooth : mouse_wheel)
	if (mouse_pinch != 0)
		zoom_delta -= mouse_pinch * 4

	if (((((window_busy = "" || window_busy = "place") && content_mouseon) || (window_busy = "viewrotatecamera" && window_focus = string(view)))) && zoom_delta <> 0)
	{
		var zoom_factor = power(1.25, zoom_delta)
		if (!cam)
			cam_work_zoom_goal = clamp(cam_work_zoom_goal * zoom_factor, cam_near, cam_far)
		else if (cam.value[e_value.CAM_ROTATE] && editcamobj)
		{
			...
			cam.cam_goalzoom = max(1, cam.cam_goalzoom * zoom_factor)
		}
	}
